...
function cleanupUserData(userId) {
  ...
  // 清理API配置状态
  apiConfigStates.delete(userIdStr)
  
  // 清理监控状态
  accountMonitors.delete(userIdStr)
...
export async function cleanUp() {
  ...
  for (const [key, monitorInfo] of accountMonitors.entries()) {
    if (monitorInfo && monitorInfo.abort) {
      monitorInfo.abort()
    }
  }
  accountMonitors.clear()
  
  // 关闭浏览器
  await closeBrowser()
  
  console.log('[多用户微信机器人] 资源清理完成')
}
...
const API_HEALTH_EXPIRE = 10 * 60 * 1000 // 10分钟后清除过期记录

// 定期清理过期的API健康记录
function cleanupExpiredApiHealth() {
  const now = Date.now()
  for (const [url, info] of apiHealth.entries()) {
    if (now - info.lastCheck > API_HEALTH_EXPIRE) {
...
15. 只回应当前用户说的话，别主动提别的`
  ...
  if (aiResponse && aiResponse.trim()) {
    ...
    const nextTimeMatch = finalResponse.match(/\[下次主动发消息\s*:\s*(\d+)\s*分钟/i)
    if (nextTimeMatch) {
    ...
    if (!finalResponse || !finalResponse.trim()) {
      finalResponse = aiResponse.replace(/\[重要\s*:\s*(true|false)\]/i, '').replace(/\[下次主动发消息\s*:\s*\d+\s*分钟/i, '').trim()
    }
