...
import { setPaused, getPaused } from './config.js'
...
export async function processQQCommand(e, pluginInstance, msg) {
  ...
  if (text === '#暂停微信机器人' || text === '#暂停机器人' || text === '#暂停') {
    await handlePause(e, pluginInstance)
    return true
  }
  
  if (text === '#恢复微信机器人' || text === '#恢复机器人' || text === '#恢复') {
    await handleResume(e, pluginInstance)
    return true
  }
...
async function handlePause(e, pluginInstance) {
  setPaused(true)
  await pluginInstance.reply('⏸️ 已暂停微信机器人消息处理\n系统壁纸加载等操作会更流畅')
}

async function handleResume(e, pluginInstance) {
  setPaused(false)
  await pluginInstance.reply('▶️ 已恢复微信机器人消息处理')
}